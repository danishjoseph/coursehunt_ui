...
export class CourseRequestsComponent implements OnInit {
  ...
  initTable(){
    this.editorApiService.getCourseRequestTable().subscribe((res: any) => {
  ...
  onCourseActivate(event: any){
    const formData = new FormData();
    
    formData.append('_id', event._id);

    
    this.editorApiService.activateCourse(formData).subscribe((res:any)=>{
      this.initTable();
    })
  }
